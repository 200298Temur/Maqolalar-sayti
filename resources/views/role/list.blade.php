<x-app-layout>
    <x-slot name="header">
        <div class="flex justify-between">
            ...
            @if (auth()->user()->permissionsKey()->contains('role_create'))
                    ...
                    @if ($roles->count() > 0)
                        ...
                        <tr class="border-b">
                            ...
                            <td class="px-6 py-4 text-center"> 
                                @if (auth()->user()->permissionsKey()->contains('role_edit'))
                                    <a href="{{ route('role.edit',[$role->id]) }}" class="bg-indigo-700	 text-sm rounded-md text-white px-3 py-2 hover:bg-indigo-500">@lang('message.edit')</a>                                 
                                @endif
                                @if(auth()->user()->permissionsKey()->contains('role_create'))
